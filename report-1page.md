...
## Phân công thực hiện
Ngô Gia Bảo: Phụ trách phát triển sender.py, thiết kế cấu trúc Header 20-byte, xây dựng logic mã hóa trong des_socket_utils.py và soạn thảo threat-model-1page.md.

Nguyễn Anh Dũng: Phụ trách phát triển receiver.py, xử lý hàm đọc dữ liệu chính xác recv_exact, thực hiện toàn bộ các ca kiểm thử tự động (pytest) và quản lý hệ thống logs.
...
## Kết quả
THệ thống đã vượt qua toàn bộ 6/6 ca kiểm thử tự động, bao gồm các kịch bản quan trọng:
...
Minh chứng: Kết quả chi tiết đã được lưu trữ tại thư mục logs/ (File 01-happy-path-Son.txt và 02-happy-path-Phuong.txt).
## Kết luận
